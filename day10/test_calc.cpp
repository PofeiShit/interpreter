...
{
	const int len = 15;
	std::string text[] = {"234", "3.14", "*", "DIV", "/", "+", "-", "(", ")", ":=", ".", "number", ";", "BEGIN", "END"};
	std::string token_type[] = {INTEGER_CONST, REAL_CONST, MUL, INTEGER_DIV, FLOAT_DIV, PLUS, MINUS, LPAREN, RPAREN, ASSIGN, DOT, ID, SEMI, BEGIN, END};
	std::string token_value[] = {"234", "3.14", "*", "DIV", "/", "+", "-", "(", ")", ":=", ".", "NUMBER", ";", "BEGIN", "END"};
	for(int i = 0; i < len; i++){
...
void test_program()
{
		...
		assert(interpreter.GLOBAL_SCOPE.size() == 6);
		...
		assert(atoi(interpreter.GLOBAL_SCOPE["C"].c_str()) == 27);
		assert(atoi(interpreter.GLOBAL_SCOPE["X"].c_str()) == 11);
