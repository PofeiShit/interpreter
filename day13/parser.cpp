...
std::vector<std::vector<AST*> > Parser::declarations()
{
		...
		while(1){
				...
				else if(current_token.type == PROCEDURE){
						eat(PROCEDURE);
						std::string procedure_name = current_token.value;
						eat(ID);
						eat(SEMI);
						AST* block_node =  block();
						ProcedureDecl* procedure_decl = new ProcedureDecl(procedure_name, block_node);
						std::vector<AST*> t;
						t.push_back(static_cast<AST*>(procedure_decl));
